...
import { Save, Plus, Trash2, Hash, CalendarIcon } from 'lucide-react';
...
export default function CreatePenjualanPage() {
  ...
  const [details, setDetails] = useState<{ barang_id: number; qty: number; harga_jual: number }[]>([{ barang_id: 0, qty: 1, harga_jual: 0 }]);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    api.getBarang('per_page=9999').then(r => setBarang(r.data)).catch(() => toast.error('Gagal memuat data barang'));
    api.getPenjualanNextNumber().then(r => setNoInvoice(r.no_invoice)).catch(() => {});
  }, []);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.createPenjualan({
        no_invoice: noInvoice || undefined, tanggal, konsumen: konsumen || undefined, telepon: telepon || undefined,
      ...
      const msg = e?.response?.data?.message || e?.message || 'Gagal membuat Penjualan';
      toast.error(msg);
    }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Buat transaksi Penjualan baru</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <Button onClick={onSubmit} disabled={saving}>
            <Save className="size-4" />{saving ? 'Menyimpan...' : 'Simpan Penjualan'}
          </Button>
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center rounded-lg border border-input bg-muted">
                      <div className="flex items-center justify-center pl-2 text-muted-foreground"><Hash className="size-4" /></div>
                      <input
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center rounded-lg border border-input bg-transparent has-focus-visible:border-ring has-focus-visible:ring-3 has-focus-visible:ring-ring/50">
                      <input
                        type="date"
                        className="h-8 flex-1 rounded-none border-0 bg-transparent px-2.5 py-1 text-sm outline-none"
                        value={tanggal}
                ...
                {details.map((d, i) => (
                  <div key={i} className="grid min-w-0 items-center gap-2 rounded-lg border p-2 text-sm md:border-0 md:p-0" style={{ gridTemplateColumns: 'minmax(0,1fr) 56px 96px 80px 24px' }}>
                    ...
                    <Input
                      ...
                      value={d.qty}
                      onChange={e => updateDetail(i, 'qty', Number(e.target.value))}
                    />
                    <Input
                      type="number"
                      step="1"
                      className="h-8 text-center text-sm"
                      value={d.harga_jual || ''}
                      onChange={e => updateDetail(i, 'harga_jual', Number(e.target.value))}
                    />
                    <div className="text-sm font-medium text-center">
                      {formatRp(d.qty * d.harga_jual)}
                    </div>
                    {details.length > 1 && (
